  ...
  <div class="mx-calendar">
    ...
    <div class="mx-calendar-content">
      <table class="mx-calendar-table" v-show="currentPanel === 'date'">
        ...
        <tbody>
          <tr v-for="(row, x) in dates" :key="x">
            <td v-for="(cell,x) in row" :key="x" :title="cell.title" :class="getDateClasses(cell)" @click="selectDate(cell)">{{cell.day}}</td>
          </tr>
        </tbody>
      </table>
      <div class="mx-calendar-year" v-show="currentPanel === 'years'">
        <a v-for="(year,x) in years" :key="x" @click="selectYear(year)" :class="{'current': currentYear === year}">{{year}}</a>
      </div>
        ...
        <div v-if="timeSelectOptions.length" class="mx-time-list-wrapper">
          <ul class="mx-time-list">
            <li class="mx-time-item mx-time-picker-item"
              ...
              v-for="(item,x) in timeSelectOptions" :key="x">
              {{item.label}}
            </li>
          </ul>
        </div>
...
export default {
  ...
  methods: {
    ...
    getDateClasses(cell) {
      ...
      const curTime = this.value
        ? new Date(this.value).setHours(0, 0, 0, 0)
        : 0;
      const startTime = this.startAt
        ? new Date(this.startAt).setHours(0, 0, 0, 0)
        : 0;
      const endTime = this.endAt
        ? new Date(this.endAt).setHours(0, 0, 0, 0)
        : 0;
    ...
    getTimeClasses(value, index) {
      ...
      switch (index) {
        case -1:
          ...
          cellTime = new Date(this.now).setHours(
            Math.floor(value / 60),
            value % 60,
            0
          );
