...
define(['utils/organ-cache', 'utils/loader', 'jstree'], function (organCache, Loader) {
  var updateLogs = {
     ...
     * @method init
     */
    init: function (defaultOrgan, rootOrgan, organType, level, hideOrganArr) {
      var me = this;
      me.organid = "";
      me.data = {};
      me.hideOrganArr = hideOrganArr || [];
      me.$element = $("#headbar-organ");
      me.$orgTree = me.$element.find('.org-tree-container');
    ...
    bindEvent: function () {
      ...
      elem.on('mousedown', '.filter-input,.arrow-down', function (event) {
        ...
        if ($target.hasClass('hide')) {
          me.$treeContainer.addClass('hide');
          $target.removeClass('hide');
          // 搜索定位已选单位
          var $orgTree = elem.find('.org-tree-container');
          me.$element.find('.jstree-search-input').val('');
          var searchResult = $($orgTree).jstree('search', $("#headbar-organ .filter-input").val(), false, false);
    ...
    renderorgTree: function (me, rootOrgan, defaultOrgan, orgList, extendOrg) {
      ...
      $orgTree.on("loaded.jstree", function (e, data) {
        ...
        var searchFunc = function () {
          ...
          if (emptyFlag) {
            searchVal = $("#headbar-organ .filter-input").val();
        ...
        // 输入框图标事件
        $('#jstree-search__input-icon').unbind();
        $('#jstree-search__input-icon').click(function () {
          searchFunc();
        })
        // 默认选择节点
      ...
      $orgTree.on("select_node.jstree", function (e, data) {
        ...
        me.$element.find('.filter-input').val(data.node.text);
        $treeContainer.addClass('hide');
        MAT.event.trigger('selectTreeNode', data.node);
      });
